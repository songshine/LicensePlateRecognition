...
#define MIN_CHAR_WIDTH 10
#define MIN_CHAR_HEIGHT 28
...
//获取二值化的字符
IplImage* GetThreshholdChar(const IplImage* img_gray,CvRect rect,int color);

//删除多余的矩形区域，返回最终的矩形数量
int DeleteRectRegion(const IplImage* bwImg,CvRect** boundRect,int boundNum,
					 int width,int height,struct borderParam bp);

int DeleteRects(CvRect** boundRect,int boundNum,int width,int height);

//添加矩形
int AddRects(const IplImage* img,CvRect** boundRect,int boundNum,
			 int width,int height,struct borderParam bp);
...
IplImage** CharacterSegmentation(const IplImage* src,int* imgNum,int color) 
{
	...
	GrayStretch(g_img_lpr_gray,g_img_lpr_gray);
	//cvShowImage("f",g_img_lpr_gray);
	pixMeans = cvMean(g_img_lpr_gray);
	switch(color)
	{
	case COLOR_BLUE:
		{

			cvThreshold(g_img_lpr_gray,g_img_lpr_bw,pixMeans+18,255,CV_THRESH_BINARY|CV_THRESH_OTSU);
			//cvThreshold(g_img_lpr_gray,g_img_lpr_thin,me+10,1,CV_THRESH_BINARY);
	...
	case COLOR_YELLOW:
		{
			...
			/*cvAdaptiveThreshold(g_img_lpr_gray,g_img_lpr_bw,255,
			CV_ADAPTIVE_THRESH_GAUSSIAN_C,CV_THRESH_BINARY,RESIZE_IMAGE_HEIGHT+1);*/
			cvThreshold(g_img_lpr_gray,g_img_lpr_bw,pixMeans-10,255,CV_THRESH_BINARY_INV);
			//cvThreshold(g_img_lpr_gray,g_img_lpr_thin,me-20,1,CV_THRESH_BINARY_INV);
			//cvNot(g_img_lpr_bw,g_img_lpr_bw);
		}
	...
	//cvShowImage("img_bw",g_img_lpr_bw);
	SmoothBinImg(g_img_lpr_bw,20,SMOOTH_WHITE_CC);
			  ...
			  CvRect** boundRect,int color)
{
	...
	{//双行黄牌
		...
		&& rightWidth > MIN_CHAR_WIDTH && rightHeight > MIN_CHAR_HEIGHT)
	{//单行牌
		//清除边界意外的像素点
	...
	/*	IplImage* img_tmp = cvCloneImage(grayImg);
		...
		if(boundNum < CHARACTER_NUM)
			boundNum = AddRects(bwImg,boundRect,boundNum,rightWidth,rightHeight,*bp);
		for(int i=0;i<boundNum;i++)
		{
...
//CvRect* ReturnCharRects2(IplImage* img_bw,struct borderParam bp,
//								int rightWidth,int rightHeight)
//{
//	int i,j,t,s;
			 ...
			 int width,int height,struct borderParam bp)
{
	...
	int lX = (*boundRect)[0].x;
	int rX = (*boundRect)[boundNum-1].x+(*boundRect)[boundNum-1].width;
	int lWidth = lX;
	int rWidth = img->width-rX;
	if(lWidth < width/2 && rWidth < width/2)
		return num;
	else if(lWidth > rWidth)
	{//左边添加一个矩形
		x = max(0,lX-width-2);
		y = 0;
		iWidth = min(lX-x,width);
		iHeight = min(img->height-y,height);
	}	
	else
	{//右边添加一个矩形
		x = min(rX+2,img->width);
		y = 0;
		iWidth = min(img->width-x,width);
		iHeight = min(img->height-y,height);
	}

	AddARect(boundRect,&num,cvRect(x,y,iWidth,iHeight));
	return num;
}
int DeleteRects(CvRect** boundRect,int boundNum,int width,int height)
{
	...
	for(i=0;i<boundNum;i++)
	{
		for(j = i+1;j<boundNum;j++)
		{
			...
			if(area == 0)
				continue;
			if(area >= (*boundRect)[i].height*(*boundRect)[i].width*0.66)
			{//矩形i在矩形j中
				DelARect(boundRect,&boundNum,i);
				continue;
			}
			if(area >= (*boundRect)[j].height*(*boundRect)[j].width*0.66)
			{//矩形j在i中
				DelARect(boundRect,&boundNum,j);
			}
		}
	...
	while(boundNum > CHARACTER_NUM)
	{
		...
		for(i=0;i<boundNum;i++)
		{
			nDis = abs((*boundRect)[0].width - width)+abs((*boundRect)[0].height - height);
			if(nDis > nMaxDis)
			{
				nMaxId = nDis;
				nMaxId = i;
...
IplImage** GetBoundImages(const IplImage* src,CvRect* boundRect,int boundNum,int color)
{
	...
	{	
		...
		charImg[i] = GetThreshholdChar(src,boundRect[i],color);
...
IplImage* GetThreshholdChar(const IplImage* img_gray,CvRect rect,int color)
{
	...
	if(NULL == img_gray) return NULL;
	img = GetRectImage(img_gray,rect);
	img_result = cvCreateImage(cvGetSize(img),IPL_DEPTH_8U,1);
	double me = cvMean(img);
	if(color == COLOR_BLUE)
		cvThreshold(img,img_result,me,255,CV_THRESH_BINARY);
	else
		cvThreshold(img,img_result,me,255,CV_THRESH_BINARY_INV);
	//先锐化一下啦
